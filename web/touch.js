...
export function register_start_handler()
{
  ...
  for (const type_str of ['mousedown', 'touchstart']) {
    document.addEventListener(type_str, function (ev) {
...
export function get_delta(ev, delta_key = '')
{
  ...
  const delta_pos = new PosTime(
      cur_pos.x - prev_pos.x, cur_pos.y - prev_pos.y, cur_pos.ts - prev_pos.ts
  );
  if (delta_key !== '') {
...
class PosTime
{
  ...
  {
    return `x=${this.x.toFixed(2)}, y=${this.y.toFixed(2)}, ts=...${this.x.toString().slice(-5)}`;
  }
